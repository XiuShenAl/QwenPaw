...
class TestForkProjectDirWiring:
    def test_apply_request_coding_project_reads_fork_project_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            workspace = Path(tmp)
            worktree = workspace / ".qwenpaw" / "worktrees" / "fork-wt"
    ...
    def test_apply_request_coding_project_fork_overrides_acp_key(self):
        """When fork_project_dir is set, validated worktree wins over ACP."""
        from qwenpaw.config.config import AgentProfileConfig

        with tempfile.TemporaryDirectory() as tmp:
            workspace = Path(tmp)
            acp_dir = workspace / "acp"
            fork_dir = workspace / ".qwenpaw" / "worktrees" / "fork"
            acp_dir.mkdir()
            ...
            assert Path(updated.coding_mode.project_dir) == fork_dir.resolve()
...
class TestAutopilotGatePhases:
    ...
    async def test_rejects_qa_without_forks_integrated(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # Preserve target phase; resume after merge (no exec replay).
...
class TestUltraworkForkGate:
    async def test_done_without_merge_is_blocked(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            assert wf.read_state().get("resume_phase") == "done"
...
class TestTeamForkGate:
    async def test_verify_without_merge_is_blocked(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            assert wf.read_state().get("resume_phase") == "verify"
            assert "BLOCKED" in gate.build_continuation()
...
class TestSpawnForkedAcpKey:
    async def test_spawn_forked_writes_acp_coding_project_key(
        ...
        monkeypatch.setattr(
            "qwenpaw.agents.fork_project.register_fork",
            lambda *_a, **_k: True,
        )
        monkeypatch.setattr(
            "qwenpaw.agents.fork_project.bind_fork_task",
            lambda *_a, **_k: True,
        )
        ...
        assert rc["fork_project_dir"] == str(worktree)
        assert rc[ACP_CODING_PROJECT_META_KEY] == str(worktree)
